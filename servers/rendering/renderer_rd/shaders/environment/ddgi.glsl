...
void main() {
	...
	uint base = payload.primitive_index * 9;
	vec3 v0 = vec3(tri.data[base + 0], tri.data[base + 1], tri.data[base + 2]);
	vec3 v1 = vec3(tri.data[base + 3], tri.data[base + 4], tri.data[base + 5]);
	vec3 v2 = vec3(tri.data[base + 6], tri.data[base + 7], tri.data[base + 8]);
	...
	if (dot(normal, probe_ray_direction) > 0.0) {
		normal = -normal;
	}
	...
	vec3 albedo = min(instance.albedo.rgb, vec3(0.9));
